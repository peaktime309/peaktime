...
// axios 객체 만들기
const summariesApi = axios.create({
  baseURL: `${import.meta.env.VITE_BACK_URL}/api/v1/summaries`,
}); // BASE_URL/api/vi/directories?category={category}

// axios 객체에 요청 인터셉터 추가하기 (헤더에 JWT Token 삽입하기)
summariesApi.interceptors.request.use(
  (config) => {
...
summariesApi.interceptors.response.use(
  (response) => response,
  (error) => {
    //권한 오류 발생 시
